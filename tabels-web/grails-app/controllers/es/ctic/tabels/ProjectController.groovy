...
class ProjectController {
    ...
    def sendFeedback = {
        def msg='(email: '+params.email+') sent this comment about '+params.issue+': \n' + params.message
        sendMail {     
            to "alfonso.noriega@fundacionctic.org"
            cc "ivan.minguez@fundacionctic.org"
    ...
    def rename = {
        ...
        }catch (ProjectInvalidNameException e){ 
            log.error("While trying to rename project ${oldProjectId}", e)
            flash.error = "msg.project.not.renamed"
            flash.args = [newProjectId]
            response.status = HttpURLConnection.HTTP_BAD_REQUEST
            render(view: "index", model: indexModel(oldProjectId))
        }catch (ProjectDoesNotExistException e){ 
            log.error("While trying to rename project ${oldProjectId}", e)
    ...
    def autogenerateProgram = {
        ...
        try {
            ...
            redirect(action: "index", params: [id: projectId])
        } catch (ProjectDoesNotExistException e) {
            log.error("While trying to access project ${e.projectId}", e)
			flash.error = "msg.project.not.found"
			flash.args = [projectId]
	...
	def exhibitData = {
	    ...
	    try {
    	    render(contentType: "text/json") {
    	        items = array {
    	            for (resource in projectService.getResources(projectId).take(1000)) {
    	                item resource
    	            }
    	        }
    	    }
