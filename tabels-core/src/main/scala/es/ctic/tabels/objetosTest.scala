...
object TEMP1 extends Template(Set(TripleTemplate(Right(Variable("?x")),Left(Resource("http://example/name")), Right(Variable("?name"))),TripleTemplate(Right(Variable("?x")),Left(Resource("http://example/type")), Left(Resource("http://foaf:person")))))
