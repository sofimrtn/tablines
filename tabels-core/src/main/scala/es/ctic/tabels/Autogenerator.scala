...
class ScovoAutogenerator(defaultNamespace : Namespace = EX, projectId: String  = "DefaultTabels") extends Autogenerator with Logging {
    ...
    override def autogenerateProgram(dataSource : DataSource) : S = {
        ...
        val letItemStmt = LetStatement(item, ResourceExpression(ConcatExpression(Seq(GetRowExpression(rowId),VariableReference(Variable("?_files")))), my()), nestedStatement =Some(matchRowStmt))
...
class mapLabAutogenerator(defaultNamespace : Namespace = EX, projectId: String = "DefaultTabels") extends Autogenerator with Logging {
  ...
  override def autogenerateProgram(dataSource : DataSource) : S = {
    ...
    val inDataSheetStmt = SetInDimensionStatement(Dimension.sheets, fixedDimension = sheet, nestedStatement = Some(forDataStmt))
   ...
   /*Last triple property being replaced by neogeo:geometry*/
    ...
    tripleTemplates += TripleTemplate(typeResource,SKOS("inScheme"),my("collection"))
    
    val templateType = Template(tripleTemplates)
    ...
    val templates = List(dcatTriples.dataSetMetadataTemplate(defaultNamespace,my), dcatTriples.dataSetRDFDistributionMetadataTemplate(defaultNamespace,my),dcatTriples.dataSetTurtleDistributionMetadataTemplate(defaultNamespace,my), dcatTriples.dataSetN3DistributionMetadataTemplate(defaultNamespace,my), templateFeature,templateType, propertyDefinitionTemplate)
