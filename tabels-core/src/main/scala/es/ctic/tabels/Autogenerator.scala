...
class BasicAutogenerator(defaultNamespace : Namespace = EX) extends Autogenerator with Logging {
    ...
    val prefixes = Seq(("ex", defaultNamespace()), ("rdf", RDF()), ("rdfs", RDFS()),("dcat", DCAT()), ("dct", DCT()), ("foaf", FOAF()))
...
class ScovoAutogenerator extends Autogenerator with Logging {
    ...
    override def autogenerateProgram(dataSource : DataSource) : S = {
        ...
        templates += dcatTriples.catalogMetadataTemplate()
        templates += dcatTriples.catalogPublisherMetadataTemplate()
        templates += dcatTriples.dataSetMetadataTemplate()
