...
class JenaDataOutput(prefixes : Map[String,NamedResource] = Map()) extends DataOutput with Logging {
  ...
  def createObject(s : RDFNode) : com.hp.hpl.jena.rdf.model.RDFNode = {
    s match {
    	...
    	case Literal(value, XSD_INTEGER, _) => model.createTypedLiteral(value, XSDDatatype.XSDinteger)
    	case Literal(value, XSD_LONG, _) => model.createTypedLiteral(value, XSDDatatype.XSDlong)
